...
static const uint8_t init_cmds[] = {
		...
		ST7789_RASET  , 4,                 	// 6: Row addr set, 4 args, no delay:
		  ...
		  ST7789_INVOFF ,   DELAY,     		// 7: Inversion ON ST7789_INVOFF ST7789_INVERSION
		  10,
...
//				buff_frame[( y + i ) * ST7789_Width + x + j] = *data; //ST7789_Width
//				data++;
//			}
//
//		}
    	for (uint16_t i = 0; i < h; i++) {
    		for (uint16_t j = 0; j < w; j++) {
    			uint16_t pix = *data++;           // RRRRRGGGGGGBBBBB
    			pix = __REV16(pix);               // поменять байты местами
    			buff_frame[(y+i)*ST7789_Width + x + j] = pix;
    		}
    	}
